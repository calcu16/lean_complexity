...
inductive witness
  (m: complexity.model): Π {α β: Type*}, m.Program → α → β → Prop
...
structure complexity {α β: Type*} (m: complexity.model) (f: α) (cost: β) :=
  (program: m.Program)
  (pf: complexity.witness m program f cost)